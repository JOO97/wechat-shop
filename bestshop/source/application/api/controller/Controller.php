...
class Controller extends ThinkController
{
    ...
    {
        if (!$wxapp_id = $this->request->param('wxapp_id')) {//获取请求数据中的wxapp_id
            throw new BaseException(['msg' => '缺少wxapp_id']);
        }
        return $wxapp_id;
    }
    ...
    protected function getUser()
    {
        if (!$token = $this->request->param('token')) {
            throw new BaseException(['code' => -1, 'msg' => '缺少token']);
        }
        if (!$user = UserModel::getUser($token)) {
            throw new BaseException(['code' => -1, 'msg' => '该用户不存在']);
        }
        return $user;
    }
